...
# ggplot2 es una librería gráfica que funciona por capas. Es un elegante,
...
# =========================================================================== #
# Ejemplo anterior con titulos y demás:

iris %>%  ggplot(aes(x=factor(Species) , y= Sepal.Length))+
          geom_boxplot(fill = c("yellow","pink","green"), # para rellenar
                       color="blue")+                     # Color del borde
          ggtitle ("Box Plot") +
          labs(x = "Religiones", y = "Cantidad")         # Etiquetas ejes.
...
h2 <- iris %>% ggplot(aes(x=Sepal.Length))+
  geom_histogram(bins=20,               # Cántidad de los bins.
...
ggplot(tp,                               # llamar a la tabla de datos.
